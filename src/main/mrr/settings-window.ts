...
function registerHandlers(deps: SettingsWindowDeps): void {
  ...
  ipcMain.handle(SETTINGS_READ_STATUS_CHANNEL, (event) => {
    if (!isFromSettingsWindow(event)) return { error: 'unauthorized' };
    const s = deps.getSettings();
    return { stripeConnected: s.stripeConnected, revenuecatConnected: s.revenuecatConnected, mode: s.mode };
  });

  ipcMain.handle(SETTINGS_SAVE_CHANNEL, async (event, payload: unknown) => {
    if (!isFromSettingsWindow(event)) return { ok: false, error: 'unauthorized' };
    ...
    const requestedMode = parsed.mode ?? current.mode;
    // Never trust the renderer's disabled-radio UI alone: mode can only be
    // 'mrr' if at least one source is connected after this save.
    const mode: Mode = requestedMode === 'mrr' && !(stripeConnected || revenuecatConnected) ? 'tokens' : requestedMode;
  ...
  ipcMain.handle(SETTINGS_DISCONNECT_CHANNEL, async (event, payload: unknown) => {
    if (!isFromSettingsWindow(event)) return { ok: false, error: 'unauthorized' };
    ...
    const mode: Mode = stripeConnected || revenuecatConnected ? current.mode : 'tokens';
    const next: SettingsState = { mode, stripeConnected, revenuecatConnected };
    saveSettingsState(deps.stateDir, next);
    deps.onSettingsChanged(next);
    return { ok: true };
