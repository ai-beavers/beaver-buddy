...
describe('xp store', () => {
  it('missing file -> fresh state, no crash', () => {
    expect(loadState(stateDir)).toEqual({ xp: 0, lastSeenLifetimeTokens: 0 });
  });

  it('roundtrips a saved state', () => {
    const state = { xp: 1234.5, lastSeenLifetimeTokens: 987654 };
  ...
  it('creates the state dir if missing', () => {
    ...
    saveState(nested, { xp: 1, lastSeenLifetimeTokens: 2 });
    expect(loadState(nested)).toEqual({ xp: 1, lastSeenLifetimeTokens: 2 });
  });

  it('leaves no stray tmp files behind after a save (atomic tmp cleanup)', () => {
    saveState(stateDir, { xp: 10, lastSeenLifetimeTokens: 20 });
    saveState(stateDir, { xp: 30, lastSeenLifetimeTokens: 40 });
