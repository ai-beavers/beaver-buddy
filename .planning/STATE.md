...
**Next:** Brief an external coding agent for the re-scoped M3/P1 WAVE-2 — research the hook mechanisms of the coding-agent CLIs (Claude Code first) and build the tracer bullet: hook → normalized event → main process → visible beaver reaction. Starting material and constraints are listed in `Milestone-3/Phase-1/PHASE.md`. Independent of that: M4/P1 WAVE-2 (durable daily aggregate storage) and the M5 runtime waves remain open.
