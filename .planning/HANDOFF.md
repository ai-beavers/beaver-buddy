...
- **WAVE-2 is ready to dispatch.** The full brief for the external coding agent lives in `KICKOFF-AGENT-PROMPTS.md` ("Prompt for the external coding agent — M3/P1 WAVE-2"). The old Jurij prompt is marked SUPERSEDED — it would have sent someone to redo the finished WAVE-1 under a premise the owner has since reversed. Note: a cloud agent cannot see local files, so `~/.codex/herdr-agent-state.ps1` must be pasted in if it should study Herdr's finished hook.
- **One open fragility:** that prompt reads `docs/research/*` off `bl-figure/beaver-baby` via `git show`, because those files are not on `main`. If the branch is ever merged and deleted, the reference breaks. A small PR bringing only `docs/research/` to `main` would make it stable — owner call, since it means merging to `main`.
