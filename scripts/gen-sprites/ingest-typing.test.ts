...
describe('ingest-typing committed sheet', () => {
  ...
  it('appends a type row of 8 frames to the golden adult sheet', () => {
    expect(meta.rows.map((row) => row.name)).toEqual(['idle', 'walk', 'struggle', 'parachute-wind', 'land', 'type', 'watering', 'drink', 'sleep', 'stretch', 'speak', 'throw-stick', 'collect-sticks', 'exercise']);
